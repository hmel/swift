...
def extract_non_flags(flags) :
    ...
    return (non_flags, [x for x in flags if x not in non_flags])
...
class NinjaBuilder :
    ...
    def serialize(self) :
        ...
        for prefix in list(self._flags.values()) :
...
exec(compile(open(CONFIGURATION_FILE, "rb").read(), CONFIGURATION_FILE, 'exec'))
