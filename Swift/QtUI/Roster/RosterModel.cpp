...
namespace Swift {
  ...
  QVariant RosterModel::data(const QModelIndex& index, int role) const {
    ...
    switch (role) {
      ...
      case PresenceIconRole:
        return getPresenceIcon(item);
      case ChildCountRole:
        return getChildCount(item);
      case IdleRole:
        return getIsIdle(item);
      case JIDRole:
        return getJID(item);
      case DisplayJIDRole:
        return getDisplayJID(item);
      default:
        return QVariant();
    }
  }
  ...
  QColor RosterModel::intToColor(int color) const {
    return QColor(((color & 0xFF0000) >> 16), ((color & 0xFF00) >> 8), (color & 0xFF));
  }
  ...
  QIcon RosterModel::getPresenceIcon(RosterItem* item) const {
    ...
    if (!contact)
      return QIcon();
    if (contact->blockState() == ContactRosterItem::IsBlocked || contact->blockState() == ContactRosterItem::IsDomainBlocked) {
