...
namespace Swift {
  ...
  QtVCardDescriptionField::QtVCardDescriptionField(QWidget* parent, QGridLayout* layout, bool editable) : QtVCardGeneralField(parent, layout, editable, layout->rowCount(), tr("Description"), false, false), descriptionPlainTextEdit(nullptr) {
  ...
  void QtVCardDescriptionField::handleEditibleChanged(bool isEditable) {
    ...
    if (isEditable) {
      ...
      int height = boundingRect.height() + cm.top() + cm.bottom() + inputMetrics.height();
