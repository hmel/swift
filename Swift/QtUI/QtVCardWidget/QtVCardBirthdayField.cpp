...
namespace Swift {
  ...
  QtVCardBirthdayField::QtVCardBirthdayField(QWidget* parent, QGridLayout* layout, bool editable) : QtVCardGeneralField(parent, layout, editable, layout->rowCount(), tr("Birthday"), false, false), birthdayLabel(nullptr), birthdayDateEdit(nullptr) {
  ...
  boost::posix_time::ptime QtVCardBirthdayField::getBirthday() const {
    QDateTime dt(birthdayDateEdit->date(), QTime());
    return boost::posix_time::from_time_t(dt.toSecsSinceEpoch());
  }

  void QtVCardBirthdayField::handleEditibleChanged(bool isEditable) {
    birthdayLabel->setText(birthdayDateEdit->date().toString(QLocale::system().dateTimeFormat()));
