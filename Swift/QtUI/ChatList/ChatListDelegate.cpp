...
namespace Swift {
  ...
  void ChatListDelegate::paintRecent(QPainter* painter, const QStyleOptionViewItem& option, ChatListRecentItem* item) const {
    ...
    QIcon presenceIcon = item->data(ChatListRecentItem::PresenceIconRole).isValid() && !item->data(ChatListRecentItem::PresenceIconRole).value<QIcon>().isNull() ? item->data(ChatListRecentItem::PresenceIconRole).value<QIcon>() : QIcon(":/icons/offline.png");
