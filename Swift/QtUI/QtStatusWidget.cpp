...
namespace Swift {
  ...
  void QtStatusWidget::generateList() {
    ...
    for (const auto& savedStatus : previousStatuses) {
      if (savedStatus.first.empty() || std::find_if(allTypes_.begin(), allTypes_.end(), [&](StatusShow::Type type) { return (savedStatus.second == type) && (savedStatus.first == statusShowTypeToFriendlyName(type)); }) != allTypes_.end()) {
        continue;
      }
