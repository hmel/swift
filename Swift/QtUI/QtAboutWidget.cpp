...
namespace Swift {
  ...
  QtAboutWidget::QtAboutWidget(SettingsProvider* settingsProvider, AutoUpdater* autoUpdater) : QDialog(), settingsProvider_(settingsProvider), autoUpdater_(autoUpdater) {
    ...
    connect(websiteLabel, &QLabel::linkActivated, []() { QDesktopServices::openUrl(QUrl("https://swift.im/")); });
    ...
    if (autoUpdater_) {
      ...
      autoUpdaterChangeConnection_ = autoUpdater_->onUpdateStateChanged.connect([&](AutoUpdater::State /*updatedState*/) { updateUpdateInfo(); });
    }
