 ...
 */
  void ChatsManager::handlePresenceChange(std::shared_ptr<Presence> newPresence) {
    if (mucRegistry_->isMUC(newPresence->getFrom().toBare()))
      return;
#ifndef NOT_YET
    for (ChatListWindow::Chat& chat : recentChats_) {
    ...
    if (it == chatControllers_.end())
      return;
    JID bareJID(fullJID.toBare());
    //It doesn't make sense to have two unbound dialogs.
    if (chatControllers_.find(bareJID) != chatControllers_.end())
      return;
 ...
 */
  ...
  void ChatsManager::handleIncomingMessage(std::shared_ptr<Message> incomingMessage) {
    ...
    if (message->getFrom().toBare() == jid_.toBare()) {
      ...
      if ((carbonsReceived = incomingMessage->getPayload<CarbonsReceived>()) && (forwarded = carbonsReceived->getForwarded()) && (forwardedMessage = std::dynamic_pointer_cast<Message>(forwarded->getStanza()))) {
        message = forwardedMessage;
      }
      else if ((carbonsSent = incomingMessage->getPayload<CarbonsSent>()) && (forwarded = carbonsSent->getForwarded()) && (forwardedMessage = std::dynamic_pointer_cast<Message>(forwarded->getStanza()))) {
  ...
  void ChatsManager::handleLocalServiceFound(const JID& service, std::shared_ptr<DiscoInfo> info) {
    for (DiscoInfo::Identity identity : info->getIdentities()) {
      if ((identity.getCategory() == "directory" && identity.getType() == "chatroom") || (identity.getCategory() == "conference" && identity.getType() == "text")) {
