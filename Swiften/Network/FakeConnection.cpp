...
namespace Swift {
  ...
  void FakeConnection::setError(const Error& e) {
    ...
    if (connectedTo) {
      eventLoop->postEvent(boost::bind(boost::ref(onDisconnected), error), shared_from_this());
    }
  }
  ...
  void FakeConnection::connect(const HostAddressPort& address) {
    if (delayConnect) {
      ...
      eventLoop->postEvent(boost::bind(boost::ref(onConnectFinished), error ? true : false), shared_from_this());
    }
  }
