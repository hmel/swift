...
namespace Swift {
  ...
  public:
    ...
    virtual void run() {
      ...
      retval = ub_resolve_async(ubContext, const_cast<char*>(name.c_str()), LDNS_RR_TYPE_SRV, 1 /* CLASS IN (internet) */, helper, UnboundDomainNameResolver::unbound_callback_wrapper, NULL);
      if (retval != 0) {
        ...
        if (result->havedata) {
          ...
          if (buffer && ldns_wire2pkt(&replyPacket, static_cast<const uint8_t*>(result->answer_packet), result->answer_len) == LDNS_STATUS_OK) {
            ...
            for (size_t n = 0; n < ldns_rr_list_rr_count(rrList); n++) {
              ...
              if ((ldns_rr_get_type(rr) != LDNS_RR_TYPE_SRV) || (ldns_rr_get_class(rr) != LDNS_RR_CLASS_IN) || (ldns_rr_rd_count(rr) != 4)) {
                continue;
              }
              ...
              if ((ldns_rdf2buffer_str_dname(buffer, ldns_rr_rdf(rr, 3)) != LDNS_STATUS_OK) || (ldns_buffer_position(buffer) < 2) || !ldns_buffer_reserve(buffer, 1)) {
  ...
  public:
    ...
    virtual void run() {
      ...
      retval = ub_resolve_async(ubContext, const_cast<char*>(name.c_str()), LDNS_RR_TYPE_A, 1 /* CLASS IN (internet) */, helper, UnboundDomainNameResolver::unbound_callback_wrapper, NULL);
      if (retval != 0) {
        SWIFT_LOG(debug) << "resolve error: " << ub_strerror(retval);
        delete helper;
      }
