...
namespace Swift {
  ...
  Presence::ref PresenceOracle::getHighestPriorityPresence(const JID& bareJID) const {
    ...
    for (const auto& jidPresence : i->second) {
      ...
      if (!highest || current->getPriority() > highest->getPriority() || (current->getPriority() == highest->getPriority() && StatusShow::typeToAvailabilityOrdering(current->getShow()) > StatusShow::typeToAvailabilityOrdering(highest->getShow()))) {
        highest = current;
      }
    }
    return highest;
