...
namespace Swift {
  ...
  std::string JingleSessionImpl::sendSetRequest(JinglePayload::ref payload) {
    std::shared_ptr<GenericRequest<JinglePayload>> request = std::make_shared<GenericRequest<JinglePayload>>(IQ::Set, peerJID, payload, iqRouter);
    pendingRequests.insert(std::make_pair(request, request->onResponse.connect(boost::bind(&JingleSessionImpl::handleRequestResponse, this, request))));
    return request->send();
  }
