...
namespace Swift {
  ...
  std::vector<HistoryMessage> SQLiteHistoryStorage::getMessagesFromDate(const JID& selfJID, const JID& contactJID, HistoryMessage::Type type, const boost::gregorian::date& date) const {
    ...
    if (contactJID.isBare()) {
      // match only bare jid
      selectQuery += " AND ((fromBare=" + boost::lexical_cast<std::string>(*selfID) + " AND toBare=" + boost::lexical_cast<std::string>(*contactID) + ") OR (fromBare=" + boost::lexical_cast<std::string>(*contactID) + " AND toBare=" + boost::lexical_cast<std::string>(*selfID) + ")))";
    }
    else {
      // match resource too
      selectQuery += " AND ((fromBare=" + boost::lexical_cast<std::string>(*selfID) + " AND (toBare=" + boost::lexical_cast<std::string>(*contactID) + " AND toResource='" + getEscapedString(contactJID.getResource()) + "')) OR ((fromBare=" + boost::lexical_cast<std::string>(*contactID) + " AND fromResource='" + getEscapedString(contactJID.getResource()) + "') AND toBare=" + boost::lexical_cast<std::string>(*selfID) + ")))";
    }
    ...
    if (!date.is_not_a_date()) {
      ...
      selectQuery += " AND (time>=" + boost::lexical_cast<std::string>(lowerBound) + " AND time<" + boost::lexical_cast<std::string>(upperBound) + ")";
    }
  ...
  ContactsMap SQLiteHistoryStorage::getContacts(const JID& selfJID, HistoryMessage::Type type, const std::string& keyword) const {
    ...
    std::string query = "SELECT DISTINCT messages.'fromBare', messages.'fromResource', messages.'toBare', messages.'toResource', messages.'time' "
                        ...
                        boost::lexical_cast<std::string>(type) + " AND (toBare=" + boost::lexical_cast<std::string>(*id) + " OR fromBare=" + boost::lexical_cast<std::string>(*id) + "))";
  ...
  boost::posix_time::ptime SQLiteHistoryStorage::getLastTimeStampFromMUC(const JID& selfJID, const JID& mucJID) const {
    ...
    sqlite3_stmt* selectStatement;
    std::string selectQuery = "SELECT messages.'time', messages.'offset' from messages WHERE type=1 AND (toBare=" + boost::lexical_cast<std::string>(*selfID) + " AND fromBare=" + boost::lexical_cast<std::string>(*mucID) + ") ORDER BY time DESC LIMIT 1";
