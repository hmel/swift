...
namespace Swift {
  ...
  SchannelContext::SchannelContext(bool tls1_0Workaround) : state_(Start), secContext_(0), myCertStore_(NULL), certStoreName_("MY"), certName_(), smartCardReader_(), checkCertificateRevocation_(true), tls1_0Workaround_(tls1_0Workaround), disconnectOnCardRemoval_(true) {
    contextFlags_ = ISC_REQ_ALLOCATE_MEMORY | ISC_REQ_CONFIDENTIALITY | ISC_REQ_EXTENDED_ERROR | ISC_REQ_INTEGRITY | ISC_REQ_REPLAY_DETECT | ISC_REQ_SEQUENCE_DETECT | ISC_REQ_USE_SUPPLIED_CREDS | ISC_REQ_STREAM;
  ...
  void SchannelContext::connect() {
    ...
    SECURITY_STATUS status = AcquireCredentialsHandle(NULL, UNISP_NAME, SECPKG_CRED_OUTBOUND, NULL, &sc, NULL, NULL, credHandle_.Reset(), NULL);
    ...
    status = InitializeSecurityContext(credHandle_, NULL, NULL, contextFlags_, 0, 0, NULL, 0, contextHandle_.Reset(), &outBufferDesc, &secContext_, NULL);
  ...
  SECURITY_STATUS SchannelContext::validateServerCertificate() {
    ...
    const LPSTR usage[] = {szOID_PKIX_KP_SERVER_AUTH, szOID_SERVER_GATED_CRYPTO, szOID_SGC_NETSCAPE};
    ...
    BOOL success = CertGetCertificateChain(NULL, // Use the chain engine for the current user (assumes a user is logged in)
                                           pServerCert->getCertContext(), NULL, pServerCert->getCertContext()->hCertStore, &chainParams, chainFlags, NULL, pChainContext.Reset());
    ...
    // Verify the chain
    if (!CertVerifyCertificateChainPolicy(CERT_CHAIN_POLICY_SSL, pChainContext, &certChainPolicy, &certChainPolicyStatus)) {
      return GetLastError();
    }
  ...
  void SchannelContext::continueHandshake(const SafeByteArray& data) {
    ...
    while (!receivedData_.empty()) {
      ...
      SECURITY_STATUS status = InitializeSecurityContext(credHandle_, contextHandle_, NULL, contextFlags_, 0, 0, &inBufferDesc, 0, NULL, &outBufferDesc, &secContext_, NULL);
  ...
  void SchannelContext::handleCertError(SECURITY_STATUS status) {
    if (status == SEC_E_UNTRUSTED_ROOT || status == CERT_E_UNTRUSTEDROOT || status == CRYPT_E_ISSUER_SERIALNUMBER || status == CRYPT_E_SIGNER_NOT_FOUND || status == CRYPT_E_NO_TRUSTED_SIGNER) {
      verificationError_ = CertificateVerificationError::Untrusted;
    }
    else if (status == SEC_E_CERT_EXPIRED || status == CERT_E_EXPIRED) {
    ...
    else if (status == CRYPT_E_HASH_VALUE || status == TRUST_E_CERT_SIGNATURE) {
    ...
    else if (status == CRYPT_E_NO_REVOCATION_CHECK || status == CRYPT_E_REVOCATION_OFFLINE) {
  ...
  void SchannelContext::encryptAndSendData(const SafeByteArray& data) {
    ...
    size_t messageBufferSize = (data.size() > streamSizes_.cbMaximumMessage) ? streamSizes_.cbMaximumMessage : data.size();
    ...
    do {
      ...
      size_t bytesToSend = (bytesLeftToSend > streamSizes_.cbMaximumMessage) ? streamSizes_.cbMaximumMessage : bytesLeftToSend;
