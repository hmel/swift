...
DefaultFileTransferTransporter::DefaultFileTransferTransporter(const JID& initiator, const JID& responder, Role role, SOCKS5BytestreamRegistry* s5bRegistry, SOCKS5BytestreamServerManager* s5bServerManager, SOCKS5BytestreamProxiesManager* s5bProxy, IDGenerator* idGenerator, ConnectionFactory* connectionFactory, TimerFactory* timerFactory, CryptoProvider* crypto, IQRouter* router, const FileTransferOptions& options) : initiator(initiator), responder(responder), role(role), s5bRegistry(s5bRegistry), s5bServerManager(s5bServerManager), s5bProxy(s5bProxy), crypto(crypto), router(router) {

  localCandidateGenerator = new LocalJingleTransportCandidateGenerator(s5bServerManager, s5bProxy, role == Initiator ? initiator : responder, idGenerator, options);
  localCandidateGenerator->onLocalTransportCandidatesGenerated.connect(boost::bind(&DefaultFileTransferTransporter::handleLocalCandidatesGenerated, this, _1));

  remoteCandidateSelector = new RemoteJingleTransportCandidateSelector(connectionFactory, timerFactory, options);
  remoteCandidateSelector->onCandidateSelectFinished.connect(boost::bind(&DefaultFileTransferTransporter::handleRemoteCandidateSelectFinished, this, _1, _2));
}
...
DefaultFileTransferTransporter::~DefaultFileTransferTransporter() {
  ...
  remoteCandidateSelector->onCandidateSelectFinished.disconnect(boost::bind(&DefaultFileTransferTransporter::handleRemoteCandidateSelectFinished, this, _1, _2));
  delete remoteCandidateSelector;

  localCandidateGenerator->onLocalTransportCandidatesGenerated.disconnect(boost::bind(&DefaultFileTransferTransporter::handleLocalCandidatesGenerated, this, _1));
...
void DefaultFileTransferTransporter::handleLocalCandidatesGenerated(const std::vector<JingleS5BTransportPayload::Candidate>& candidates) {
...
void DefaultFileTransferTransporter::handleRemoteCandidateSelectFinished(const boost::optional<JingleS5BTransportPayload::Candidate>& candidate, std::shared_ptr<SOCKS5BytestreamClientSession> session) {
  remoteS5BClientSession = session;
  onRemoteCandidateSelectFinished(s5bSessionID, candidate);
}

void DefaultFileTransferTransporter::addRemoteCandidates(const std::vector<JingleS5BTransportPayload::Candidate>& candidates, const std::string& dstAddr) {
...
std::shared_ptr<TransportSession> DefaultFileTransferTransporter::createIBBSendSession(const std::string& sessionID, unsigned int blockSize, std::shared_ptr<ReadBytestream> stream) {
  if (s5bServerManager->getServer()) {
    closeLocalSession();
  }
  closeRemoteSession();
  std::shared_ptr<IBBSendSession> ibbSession = std::make_shared<IBBSendSession>(sessionID, initiator, responder, stream, router);
  ibbSession->setBlockSize(blockSize);
  return std::make_shared<IBBSendTransportSession>(ibbSession);
}

std::shared_ptr<TransportSession> DefaultFileTransferTransporter::createIBBReceiveSession(const std::string& sessionID, unsigned long long size, std::shared_ptr<WriteBytestream> stream) {
  if (s5bServerManager->getServer()) {
    closeLocalSession();
  }
  closeRemoteSession();
  std::shared_ptr<IBBReceiveSession> ibbSession = std::make_shared<IBBReceiveSession>(sessionID, initiator, responder, size, stream, router);
  return std::make_shared<IBBReceiveTransportSession>(ibbSession);
}

std::shared_ptr<TransportSession> DefaultFileTransferTransporter::createRemoteCandidateSession(std::shared_ptr<ReadBytestream> stream, const JingleS5BTransportPayload::Candidate& /* candidate */) {
  closeLocalSession();
  return std::make_shared<S5BTransportSession<SOCKS5BytestreamClientSession>>(remoteS5BClientSession, stream);
}

std::shared_ptr<TransportSession> DefaultFileTransferTransporter::createRemoteCandidateSession(std::shared_ptr<WriteBytestream> stream, const JingleS5BTransportPayload::Candidate& /* candidate */) {
  closeLocalSession();
  return std::make_shared<S5BTransportSession<SOCKS5BytestreamClientSession>>(remoteS5BClientSession, stream);
}
...
std::shared_ptr<TransportSession> DefaultFileTransferTransporter::createLocalCandidateSession(std::shared_ptr<ReadBytestream> stream, const JingleS5BTransportPayload::Candidate& candidate) {
  closeRemoteSession();
...
std::shared_ptr<TransportSession> DefaultFileTransferTransporter::createLocalCandidateSession(std::shared_ptr<WriteBytestream> stream, const JingleS5BTransportPayload::Candidate& candidate) {
  closeRemoteSession();
  std::shared_ptr<TransportSession> transportSession;
  if (candidate.type == JingleS5BTransportPayload::Candidate::ProxyType) {
    SOCKS5BytestreamClientSession::ref proxySession = s5bProxy->getProxySessionAndCloseOthers(candidate.jid, getLocalCandidateSOCKS5DstAddr());
