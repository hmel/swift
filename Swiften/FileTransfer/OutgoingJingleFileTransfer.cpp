...
OutgoingJingleFileTransfer::OutgoingJingleFileTransfer(const JID& toJID, JingleSession::ref session, std::shared_ptr<ReadBytestream> stream, FileTransferTransporterFactory* transporterFactory, TimerFactory* timerFactory, IDGenerator* idGenerator, const JingleFileTransferFileInfo& fileInfo, const FileTransferOptions& options, CryptoProvider* crypto) : JingleFileTransfer(session, toJID, transporterFactory), idGenerator(idGenerator), stream(stream), fileInfo(fileInfo), options(options), contentID(idGenerator->generateID(), JingleContentPayload::InitiatorCreator), state(Initial), candidateAcknowledged(false) {
  ...
  stream->onRead.connect(boost::bind(&IncrementalBytestreamHashCalculator::feedData, hashCalculator, _1));
...
OutgoingJingleFileTransfer::~OutgoingJingleFileTransfer() {
  ...
  stream->onRead.disconnect(boost::bind(&IncrementalBytestreamHashCalculator::feedData, hashCalculator, _1));
...
void OutgoingJingleFileTransfer::handleSessionAcceptReceived(const JingleContentID&, JingleDescription::ref, JingleTransportPayload::ref transportPayload) {
  SWIFT_LOG(debug);
...
void OutgoingJingleFileTransfer::handleSessionTerminateReceived(boost::optional<JinglePayload::Reason> reason) {
  SWIFT_LOG(debug);
  if (state == Finished) {
    SWIFT_LOG(warning) << "Incorrect state: " << state;
    return;
  }
...
void OutgoingJingleFileTransfer::handleLocalTransportCandidatesGenerated(const std::string& s5bSessionID, const std::vector<JingleS5BTransportPayload::Candidate>& candidates, const std::string& dstAddr) {
  SWIFT_LOG(debug);
  if (state != GeneratingInitialLocalCandidates) {
    SWIFT_LOG(warning) << "Incorrect state: " << state;
    return;
  }
  ...
  if (candidates.empty()) {
    ...
    for (auto&& candidate : candidates) {
      ...
      SWIFT_LOG(debug) << "\t"
                       << "S5B candidate: " << candidate.hostPort.toString();
    }
...
void OutgoingJingleFileTransfer::startTransferring(std::shared_ptr<TransportSession> transportSession) {
  ...
  processedBytesConnection = transportSession->onBytesSent.connect(boost::bind(boost::ref(onProcessedBytes), _1));
  transferFinishedConnection = transportSession->onFinished.connect(boost::bind(&OutgoingJingleFileTransfer::handleTransferFinished, this, _1));
...
void OutgoingJingleFileTransfer::setFinishedState(FileTransfer::State::Type type, const boost::optional<FileTransferError>& error) {
  SWIFT_LOG(debug);
  this->state = Finished;
  onStateChanged(type);
  onFinished(error);
...
FileTransfer::State::Type OutgoingJingleFileTransfer::getExternalState(State state) {
  switch (state) {
    ...
    case GeneratingInitialLocalCandidates:
      return FileTransfer::State::WaitingForStart;
    case WaitingForAccept:
      return FileTransfer::State::WaitingForAccept;
    case TryingCandidates:
      return FileTransfer::State::Negotiating;
    case WaitingForPeerProxyActivate:
      return FileTransfer::State::Negotiating;
    case WaitingForLocalProxyActivate:
      return FileTransfer::State::Negotiating;
    case WaitingForCandidateAcknowledge:
      return FileTransfer::State::Negotiating;
    case FallbackRequested:
      return FileTransfer::State::Negotiating;
    case Transferring:
      return FileTransfer::State::Transferring;
...
void OutgoingJingleFileTransfer::stopAll() {
  ...
  switch (state) {
    case Initial:
      SWIFT_LOG(warning) << "Not yet started";
      break;
    case GeneratingInitialLocalCandidates:
      transporter->stopGeneratingLocalCandidates();
      break;
    case WaitingForAccept:
      break;
    case TryingCandidates:
      transporter->stopTryingRemoteCandidates();
      break;
    case FallbackRequested:
      break;
    case WaitingForPeerProxyActivate:
      break;
    case WaitingForLocalProxyActivate:
      transporter->stopActivatingProxy();
      break;
    ...
    case WaitForTermination:
      break;
    case Finished:
      SWIFT_LOG(warning) << "Already finished";
      break;
  }
  if (state != Initial) {
    removeTransporter();
