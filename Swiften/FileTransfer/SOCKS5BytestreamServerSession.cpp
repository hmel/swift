...
namespace Swift {
  ...
  SOCKS5BytestreamServerSession::SOCKS5BytestreamServerSession(std::shared_ptr<Connection> connection, SOCKS5BytestreamRegistry* bytestreams) : connection(connection), bytestreams(bytestreams), state(Initial), chunkSize(131072), waitingForData(false) {
  ...
  void SOCKS5BytestreamServerSession::start() {
    SWIFT_LOG(debug);
    dataReadConnection = connection->onDataRead.connect(boost::bind(&SOCKS5BytestreamServerSession::handleDataRead, this, _1));
  ...
  void SOCKS5BytestreamServerSession::startSending(std::shared_ptr<ReadBytestream> stream) {
    if (state != ReadyForTransfer) {
      SWIFT_LOG(debug) << "Not ready for transfer!";
      return;
    }

    readBytestream = stream;
    state = WritingData;
    dataAvailableConnection = readBytestream->onDataAvailable.connect(boost::bind(&SOCKS5BytestreamServerSession::handleDataAvailable, this));
    dataWrittenConnection = connection->onDataWritten.connect(boost::bind(&SOCKS5BytestreamServerSession::sendData, this));
    sendData();
  }
