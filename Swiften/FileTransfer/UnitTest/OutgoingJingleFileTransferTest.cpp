...
public:
  std::shared_ptr<OutgoingJingleFileTransfer> createTestling(const FileTransferOptions& options = FileTransferOptions().withAssistedAllowed(false).withDirectAllowed(false).withProxiedAllowed(false)) {
    ...
    return std::shared_ptr<OutgoingJingleFileTransfer>(new OutgoingJingleFileTransfer(to, std::shared_ptr<JingleSession>(fakeJingleSession), stream, ftTransportFactory, timerFactory, idGen, fileInfo, options, crypto.get()));
  }
