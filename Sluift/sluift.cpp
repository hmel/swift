...
SLUIFT_LUA_FUNCTION_WITH_HELP(Sluift, new_client,
...
SLUIFT_LUA_FUNCTION_WITH_HELP(Sluift, new_component,
...
SLUIFT_LUA_FUNCTION_WITH_HELP(Sluift, sha1, "Compute the SHA-1 hash of given data", "data  the data to hash", "") {
...
SLUIFT_LUA_FUNCTION_WITH_HELP(Sluift, sleep, "Sleeps for the given time.", "milliseconds  the amount of milliseconds to sleep", "") {
...
SLUIFT_LUA_FUNCTION_WITH_HELP(Sluift, new_uuid, "Generates a new UUID", "", "") {
  lua_pushstring(L, IDGenerator().generateID().c_str());
  return 1;
}

SLUIFT_LUA_FUNCTION_WITH_HELP(Sluift, from_xml, "Convert a raw XML string into a structured representation.", "string  the string to convert", "") {
...
SLUIFT_LUA_FUNCTION_WITH_HELP(Sluift, to_xml, "Convert a structured element into XML.", "element  the element to convert", "") {
...
SLUIFT_LUA_FUNCTION_WITH_HELP(Sluift, hexify, "Convert binary data into hexadecimal format.", "data  the data to convert", "") {
  if (!lua_isstring(L, 1)) {
    throw Lua::Exception("Expected string");
  }
  size_t len;
...
SLUIFT_LUA_FUNCTION_WITH_HELP(Sluift, unhexify, "Convert hexadecimal data into binary data.", "data  the data in hexadecimal format", "") {
  if (!lua_isstring(L, 1)) {
    throw Lua::Exception("Expected string");
  }
...
SLUIFT_LUA_FUNCTION(FS, list) {
  ...
  std::vector<boost::filesystem::path> items(i, boost::filesystem::directory_iterator());
...
SLUIFT_API int luaopen_sluift(lua_State* L) {
  ...
  lua_rawgeti(L, LUA_REGISTRYINDEX, Sluift::globals.coreLibIndex);
  std::vector<std::string> coreLibExports = boost::assign::list_of("tprint")("disco")("help")("get_help")("copy")("with")("read_file")("create_form");
